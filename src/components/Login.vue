...
const registerOrLogin = async () => {
  ...
  const res = isRegister.value ? await signUpEmailPassword(email.value, password.value)
...
<template>
  <main>
    ...
    <form @submit.prevent="registerOrLogin">
        ...
        <label class="block mb-4"><input type="checkbox" v-model="isRegister"/> Register?</label>
